package com.example.mobile.presentation.login.student
...
@Composable
fun LoginStudentView(
    navController: NavController,
    viewModel: LoginStudentViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(state.isLoginSuccessful) {
        if (state.isLoginSuccessful) {
            navController.navigate(Screen.HomeStudentScreen.route) {
                popUpTo(Screen.LoginStudentScreen.route) { inclusive = true }
            }
    ...
    Box(
        ...
        Column(
            ...
            Text(
                text = "Entre para aceder à sua conta Estudante",
                color = Text_Grey,
                fontSize = 14.sp,
...
@Composable
fun LoginPreview() {
    LoginStudentView(navController = rememberNavController())
}